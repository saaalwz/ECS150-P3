...
# make fs with fs_make.x, add two files with test_fs.x, ls with fs_ref.x
run_fs_add() {
    log "\n--- Running ${FUNCNAME} ---"

	run_tool ./fs_make.x test.fs 10
	run_tool dd if=/dev/zero of=test-file-1 bs=10 count=1
	run_tool dd if=/dev/zero of=test-file-2 bs=5000 count=1
	run_tool timeout 2 ./test_fs.x add test.fs test-file-1
	...
	corr_array+=("file: test-file-2, size: 5000, data_blk: 2")
